public class Plant {
	...
	public Plant(String csv) {
		...
		{
			throw new IllegalArgumentException("No CSV given to constructor.");
		}
		String[] parts = csv.split(","); // should have 3 parts

		if (parts.length != 3) {
			throw new IllegalArgumentException(
					"CSV string doesn't rhave requirned number of values. csv string = " + csv);
		}
		...
		try {
			...
			throw new IllegalArgumentException("CSV string doesn't have a valid temp (double) as second value in csv string = " + parts[1]);
		}
		temp = Double.parseDouble(parts[1]);
		String uses = parts[2];
